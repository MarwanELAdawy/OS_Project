...
inline void clear(struct Env* e, uint32 ID)
{
	assert(ID >= 0 && ID < (e->page_WS_max_size));
	e->ptr_pageWorkingSet[ID].virtual_address = 0;
	e->ptr_pageWorkingSet[ID].empty = 1;
	e->ptr_pageWorkingSet[ID].time_stamp = 0;
}
static struct Taskstate ts;
uint32 get_size(struct Env *e)
{
	int counter=0;
...
void rePlacement(struct Env * curenv, uint32 fault_va){
	struct Frame_Info *ptr_frame = NULL;
	allocate_frame(&ptr_frame);
	map_frame(curenv->env_page_directory,ptr_frame,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
	int ret = pf_read_env_page(curenv, (void*)fault_va);
	if (ret == E_PAGE_NOT_EXIST_IN_PF){
		if(fault_va > USER_HEAP_MAX && fault_va < USTACKTOP){
			int x = pf_add_empty_env_page(curenv, fault_va, 0);
			if (x == E_NO_PAGE_FILE_SPACE){
				panic("No Space");
			}
		}
		else{
			panic(" page not exist on page file or the stack");
		}
	}
	int i =0;
	for (;i<curenv->page_WS_max_size;i++){
		if(curenv->ptr_pageWorkingSet[i].empty){
			assert(i>=0 && i<curenv->page_WS_max_size);
			assert(fault_va >= 0 && fault_va < USER_TOP);
			curenv->ptr_pageWorkingSet[i].virtual_address = ROUNDDOWN(fault_va, PAGE_SIZE);
			curenv->ptr_pageWorkingSet[i].empty = 0;
			curenv->ptr_pageWorkingSet[i].time_stamp = 0x80000000;
			break;
		}
	}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	struct WorkingSetElement Vic;
	int vic_id;
	uint32 Min=0xFFFFFFFF;
	if(get_size(curenv) < curenv->page_WS_max_size){
		rePlacement(curenv,fault_va);
	}
	else{
		if(isPageReplacmentAlgorithmCLOCK()){
			uint32 * ptr_pgdir = curenv->env_page_directory ;
			uint32 * ptr_page_table;
			while(1==1){
				if ((ptr_pgdir[PDX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & PERM_PRESENT) == PERM_PRESENT)
				{
					ptr_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(ptr_pgdir[PDX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)])) ;
				}
				if((ptr_page_table[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & PERM_USED) !=PERM_USED){
					vic_id = curenv->page_last_WS_index;
					Vic = curenv->ptr_pageWorkingSet[curenv->page_last_WS_index];
					break;
				}
				else if ((ptr_pgdir[PDX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] & PERM_PRESENT) == PERM_PRESENT){
						ptr_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(ptr_pgdir[PDX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)])) ;
						ptr_page_table[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] |= (PERM_USED);
						ptr_page_table[PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)] &= (~PERM_USED);
					}
			...
			int i=0;
			for(;i<curenv->page_WS_max_size;++i){
				if(env_page_ws_get_time_stamp(curenv,i) < Min){
					Min=env_page_ws_get_time_stamp(curenv,i);
					vic_id= i;
					Vic=curenv->ptr_pageWorkingSet[i];
				}
			}
		}
		...
		uint32 * ptr_pgdir = curenv->env_page_directory ;
		uint32 * ptr_page_table;
		if ((ptr_pgdir[PDX(Vic.virtual_address)] & PERM_PRESENT) == PERM_PRESENT)
		{
			ptr_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(ptr_pgdir[PDX(Vic.virtual_address)])) ;
		}
		if((ptr_page_table[PTX(Vic.virtual_address)] & PERM_MODIFIED) !=PERM_MODIFIED){
			...
			uint32 *Ptr_Page_Table;
			Ptr_Frame_Info = get_frame_info(curenv->env_page_directory,(void*)Vic.virtual_address,&Ptr_Page_Table);
			pf_update_env_page(curenv, (void*)Vic.virtual_address,Ptr_Frame_Info);
			unmap_frame(curenv->env_page_directory,(void*)Vic.virtual_address);
		}
		clear(curenv,vic_id);
		rePlacement(curenv,fault_va);
